...
export async function upsertGames(games: import('@/types').MLBGame[]) {
  ...
  if (error) { console.warn('Game upsert failed — schema cache may need refresh:', error) }
}
...
export async function getDailyLog(date?: string, endDate?: string): Promise<import('@/types').DailyLogEntry[]> {
  ...
  const query = supabase.from('daily_logs')
    .select('*')
...
export async function getRecentNews(limit = 50): Promise<import('@/types').NewsItem[]> {
  ...
  const { data, error } = await supabase.from('news_items')
    ...
    .limit(limit) as { data: Record<string, unknown>[] | null; error: unknown }
...
export async function getResearchNotes(): Promise<import('@/types').ResearchNote[]> {
  const supabase = getSupabase()
  if (!supabase) return []
  const { data, error } = await supabase.from('research_notes')
    .select('*')
    .order('created_at', { ascending: false }) as { data: Record<string, unknown>[] | null; error: unknown }
  if (error) return []
