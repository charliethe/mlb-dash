...
export default function StandingsClient({ initialData }: { initialData?: any }) {
  useEffect(() => { document.title = 'Standings — MLB Research' }, [])
  return (
    <ErrorBoundary name="Standings">
      <StandingsInner initialData={initialData} />
    </ErrorBoundary>
  )
}

function StandingsInner({ initialData }: { initialData?: any }) {
  const searchParams = useSearchParams()
  const [divisions, setDivisions] = useState<DivisionStanding[]>(() => {
    if (initialData?.records?.length) {
      return [...initialData.records].sort((a: any, b: any) => {
