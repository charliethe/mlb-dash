...
import { fetchTeamRoster, fetchTodayGames, fetchTransactions, fetchStandings, fetchBulkPlayerStats, fetchTeamSchedule, fetchStandingsForTeam } from '@/lib/mlb/api'
...
export default function TeamDetailPage() {
  useEffect(() => { document.title = 'Team Detail — MLB Research' }, [])
  ...
  const [recentGames, setRecentGames] = useState<MLBGame[]>([])
  const [standing, setStanding] = useState<TeamStandingInfo | null>(null)
  const [divisionStandings, setDivisionStandings] = useState<{ division: string; records: { teamId: number; name: string; abbreviation: string; wins: number; losses: number; pct: string; gb: string; streak: string; l10: string }[] } | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    setError(false)
    try {
      const [r, n, t, g, rawStandings] = await Promise.all([
      ...
      const records = rawStandings.records || []
      ...
      const [divStandings, sched] = await Promise.all([
        team ? fetchStandingsForTeam({ id: teamId, league: team.league, division: team.division }) : Promise.resolve(null),
        fetchTeamSchedule(teamId, new Date(Date.now() - 14 * 86400000).toISOString().split('T')[0], new Date().toISOString().split('T')[0]),
      ])
      setDivisionStandings(divStandings)
      setRecentGames(sched.filter((sg) => {
        const isToday = sg.gameDate.split('T')[0] === new Date().toISOString().split('T')[0]
        return !isToday
      }).reverse())
    } catch (err) {
          ...
          {divisionStandings && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">{divisionStandings.division} Standings</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="text-[11px] py-1 w-6">#</TableHead>
                      <TableHead className="text-[11px] py-1">Team</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">W</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">L</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">Pct</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">GB</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">L10</TableHead>
                      <TableHead className="text-[11px] py-1 text-right">Strk</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {divisionStandings.records.map((rec, i) => {
                      const isCurrent = rec.teamId === teamId
                      return (
                        <TableRow key={rec.teamId} className={`text-xs ${isCurrent ? 'bg-muted/20 font-medium' : ''}`}>
                          <TableCell className="py-1.5 text-muted-foreground">{i + 1}</TableCell>
                          <TableCell className="py-1.5">
                            <div className="flex items-center gap-1.5">
                              {TEAM_LOGOS[rec.abbreviation] && (
                                <LogoImage src={TEAM_LOGOS[rec.abbreviation]} alt={rec.abbreviation} className="h-4 w-4 shrink-0" />
                              )}
                              <Link href={`/teams/${rec.teamId}`} className={`hover:underline ${isCurrent ? 'text-foreground' : 'text-muted-foreground'}`}>
                                {rec.abbreviation}
                              </Link>
                            </div>
                          </TableCell>
                          <TableCell className="py-1.5 text-right font-mono">{rec.wins}</TableCell>
                          <TableCell className="py-1.5 text-right font-mono">{rec.losses}</TableCell>
                          <TableCell className="py-1.5 text-right font-mono">{rec.pct}</TableCell>
                          <TableCell className="py-1.5 text-right font-mono">{rec.gb === '-' ? '—' : rec.gb}</TableCell>
                          <TableCell className="py-1.5 text-right font-mono">{rec.l10}</TableCell>
                          <TableCell className={`py-1.5 text-right font-mono ${rec.streak.startsWith('W') ? 'text-green-400' : rec.streak.startsWith('L') ? 'text-red-400' : ''}`}>
                            {rec.streak}
                          </TableCell>
                        </TableRow>
                      )
              ...
              <CardContent className="p-0">
                <div className="divide-y divide-border">
                  {recentGames.map((game) => {
                    ...
                    const isWin = isFinal && ourScore != null && theirScore != null && ourScore > theirScore
                    const opponentLogo = TEAM_LOGOS[opponent.abbreviation]

                    return (
                      <div key={game.gamePk} className="flex items-center gap-2 px-4 py-2 text-xs">
                        <span className="text-muted-foreground shrink-0 w-12">{format(parseISO(game.gameDate), 'M/d')}</span>
                        <LogoImage src={opponentLogo} alt={opponent.abbreviation} className="h-4 w-4 shrink-0" />
                        <span className="shrink-0 text-muted-foreground">{isHome ? 'vs' : '@'}</span>
                        <span className="font-medium truncate">{opponent.abbreviation}</span>
                        {isFinal && ourScore != null && theirScore != null ? (
                          <span className={`ml-auto font-mono shrink-0 ${isWin ? 'text-green-400' : 'text-red-400'}`}>
                            {ourScore}-{theirScore}
                          </span>
                        ) : (
                          <span className="ml-auto text-muted-foreground shrink-0">
                            {game.status.detailedState === 'Postponed' ? 'PPD' : format(parseISO(game.gameDate), 'h:mm a')}
                          </span>
                        )}
                      </div>
                    )
