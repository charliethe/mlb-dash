...
import { ArrowLeft, MapPin, StickyNote, Ruler, Weight, Cake, Calendar, Shirt, Award } from 'lucide-react'
...
export default function PlayerDetailPage() {
  ...
  useEffect(() => {
    document.title = player ? `${player.fullName} — MLB Research` : 'Player Detail — MLB Research'
  }, [player])
  ...
  if (fetchError) {
    return (
      <ErrorBoundary name="PlayerDetail">
        <ErrorState message="Failed to load player data" onRetry={() => window.location.reload()} />
      </ErrorBoundary>
    )
  }
  if (!player) {
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          {imgError ? (
            <div className="h-16 w-16 rounded-full bg-muted flex items-center justify-center text-lg font-medium text-muted-foreground shrink-0">
          ...
          {logo && (
            <img
              src={logo}
              alt={player.currentTeam?.abbreviation || ''}
              className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-background border border-border"
            />
          )}
        </div>
        <div className="flex-1">
          ...
          <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
            ...
            {player.primaryNumber && (
              <>
                <span>·</span>
                <span>#{player.primaryNumber}</span>
              </>
            )}
          </div>
        </div>
            ...
            <CardContent className="space-y-2 text-sm">
              {player.currentAge && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground"><Cake className="h-3 w-3" /> Age</span>
                  <span className="font-medium">{player.currentAge}{player.birthDate ? ` (${format(parseISO(player.birthDate), 'MMM d, yyyy')})` : ''}</span>
                </div>
              )}
              {player.height && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground"><Ruler className="h-3 w-3" /> Height</span>
                  <span className="font-medium">{player.height}</span>
                </div>
              )}
              {player.weight && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground"><Weight className="h-3 w-3" /> Weight</span>
                  <span className="font-medium">{player.weight} lbs</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="flex items-center gap-1 text-muted-foreground"><Shirt className="h-3 w-3" /> Bats/Throws</span>
                <span className="font-medium">{player.bats === 'R' ? 'Right' : player.bats === 'L' ? 'Left' : 'Switch'} / {player.throws === 'R' ? 'Right' : 'Left'}</span>
              </div>
              {player.currentTeam && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Team</span>
                  <span className="font-medium">{player.currentTeam.name} ({player.currentTeam.abbreviation})</span>
                </div>
              )}
              <div className="flex justify-between">
              ...
              {player.primaryNumber && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">#{player.primaryNumber}</span>
                </div>
              )}
              {(player.birthCity || player.birthCountry) && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Born</span>
                  <span className="font-medium">{[player.birthCity, player.birthCountry].filter(Boolean).join(', ')}</span>
                </div>
              )}
              {player.mlbDebutDate && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground"><Calendar className="h-3 w-3" /> MLB Debut</span>
                  <span className="font-medium">{format(parseISO(player.mlbDebutDate), 'MMM d, yyyy')}</span>
                </div>
              )}
              {player.draftYear && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground"><Award className="h-3 w-3" /> Draft</span>
