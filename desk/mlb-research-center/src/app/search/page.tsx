...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    fetch(`https://statsapi.mlb.com/api/v1/people/search?search=${encodeURIComponent(query)}`)
      .then((r) => r.ok ? r.json() : { people: [] })
      .then((data) => {
        const people = (data.people || []).slice(0, 20).map((p: { id: number; fullName: string; primaryPosition?: { abbreviation: string }; currentTeam?: { abbreviation: string } }) => ({
          id: p.id,
          fullName: p.fullName,
