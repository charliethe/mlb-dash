...
export default function SlateClient({ initialData }: { initialData?: MLBGame[] }) {
  useEffect(() => { document.title = 'Slate — MLB Research' }, [])
  const today = useMemo(() => new Date().toISOString().split('T')[0], [])
  const [games, setGames] = useState<MLBGame[]>(initialData || [])
  ...
  useEffect(() => {
    if (initialData && slateDate === today) return
    loadGames(slateDate)
  }, [slateDate])
